...
<head>
    ...
    <script>
        function addVer() {
            window.location = "" + document.form1.vertragstr.value;
        }
        ...
        function back() {
            window.location = "residenceDetails?residenceid=" + document.form1.residentids.value;
        }
...
<%
    ...
    try{
        ...
        if(resident.isStudent()==1){
            ...
            vertrag = "../../../../ResidentVertrag/attachVertrag?userid="+resident.getStudentReference().getUserID();
        }else{
            firstname=resident.getUserFirstName();
            lastname=resident.getUserLastName();
            updatelink = "residentUpdateEntry?residentid="+residentid;
            vertrag = "../../../../ResidentVertrag/attachVertrag?residentid="+residentid;
        }

        //residenceMieten=mdb.getResidenceMietenByMieter(residentid);
        residence=rdb.getResidence(resident.getUserID());
        if(residence.getResidenceID()>0){
            mark=residence.getResidenceMark();
            mietstart= MyDate.formateDate(resident.getMietStart(),MyDate.NUMERIC,MyDate.DEUTSCH);
            if(residenceMieten.getMietEnd()!=null){
                mietend=MyDate.formateDate(resident.getMietEnd(),MyDate.NUMERIC,MyDate.DEUTSCH);
            }
        }

        remarks=resident.getUserRemarks();
        session.setAttribute("residentid",resident);
    }catch(Exception e){
    }
%>
                                        ...
                                        <tr valign="top" bgcolor="AEBACD">
                                            <td colspan="3" height="25">
                                                <form style="width: 100%;" name="form1" method="post" action="onlyRemark">
                                                    ...
                                                    <input name="residentids" type="hidden" id="residentids" value="<%=resident.getUserID()%>">
                                                    <input name="updatestr" type="hidden" id="updatestr" value="<%=updatelink%>">
                                                    <input name="vertragstr" type="hidden" id="vertragstr" value="<%=vertrag%>">
                                                </form>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td height="26" colspan="3" bgcolor="AEBACD">
                                                <input class="btn btn-primary" type="button" name="Button" value="speichern" onClick="updateR('Wollen Sie die bemerkung speisen?');return false">
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="3">&nbsp;
