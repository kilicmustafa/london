...
<%
    ...
    myTr=trdb.getComplaintSearch1(Integer.parseInt(cri),reqAdmin.getUserID(),d1,d2,placeID,tranNo,1);
    pageStatus=trdb.COMPLAINTNOTACTIVE;
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="wrapper" style="margin-bottom: 5px; float:right;">
                                ...
                                <a style="margin-right: 5px;" class="btn btn-primary" href="showComplaint?status=1&tranNo=<%=tranno%>&selPlace=<%=placename%>&selC=<%=cri%>&selAdmin=<%=adminS%>&selSDay=<%=sDay%>&selSMonth=<%=sMonth%>&selSYear=<%=sYear%>&selEDay=<%=eDay%>&selEMonth=<%=eMonth%>&selEYear=<%=eYear%>">erledigt</a>
                                <input name="status" type="hidden" id="status" value="notactive">
                                <a style="margin-right: 5px;" class="btn btn-primary" href="showComplaint?status=0&tranNo=<%=tranno%>&selPlace=<%=placename%>&selC=<%=cri%>&selAdmin=<%=adminS%>&selSDay=<%=sDay%>&selSMonth=<%=sMonth%>&selSYear=<%=sYear%>&selEDay=<%=eDay%>&selEMonth=<%=eMonth%>&selEYear=<%=eYear%>">Nicht erledigt</a>
                                ...
                                <tr>
                                    <td valign="top" height="146" bgcolor="D9E5F4" width="81%"><table width="100%" height="81" border="0" cellpadding="1" cellspacing="1">
                                        ...
                                        <tr bgcolor="657A9A">
                                            ...
                                            <td width="15%" height="10" colspan="-2"><font size="2" color="#FFFFFF">&nbsp;</font><font size="2" color="#FFFFFF"><b><%=myTr.size()%></b></font></td>
                                        </tr>
                                        <tr bgcolor="657A9A">
                                        ...
                                        <tr bgcolor="AEBACD">
                                            ...
                                            <td height="10"><div align="left"><font color="#000033" size="2"><a href="changeStudentCompStatusSearch?wid=<%=ec.getComStudentID()%>&status=<%=trdb.COMPLAINTACTIVE%>&tranNo=<%=tranno%>&selPlace=<%=placename%>&selC=<%=cri%>&selAdmin=<%=adminS%>&selSDay=<%=sDay%>&selSMonth=<%=sMonth%>&selSYear=<%=sYear%>&selEDay=<%=eDay%>&selEMonth=<%=eMonth%>&selEYear=<%=eYear%>">erledigt</a></font></div></td>
                                        </tr>
                                        <%
                                        ...
                                        <tr bgcolor="AEBACD">
                                            ...
                                            <td><font size="2" color="#000033"><%=loginuser.getUserFirstName()+ ' ' + loginuser.getUserLastName()%></font></td>
                                            <td><font size="2" color="#000033"><%=myPlace.getPlaceName()%></font></td>
                                            <td height="10"><div align="left"><font color="#000033" size="2"><a href="changeStudentCompStatus?wid=<%=ec.getComStudentID()%>&status=<%=trdb.COMPLAINTNOTACTIVE%>&userid=<%=tranNo%>">Nicht erledigt</a></font></div></td>
                                        </tr>
                                        <%
                                                }
                                            }
